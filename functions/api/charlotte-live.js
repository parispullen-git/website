/* Read/write for the real Charlotte Map -- data/charlotte-locations.json,
   which build_charlotte.py bakes into charlotte.html. See journal-live.js
   for the fuller explanation this mirrors: GitHub Contents API only,
   never the live site directly, and Deploy Live still has to run to
   publish. */
...
const PATH = 'data/charlotte-locations.json';
...
export async function onRequest(context) {
  ...
  if (event.httpMethod === 'POST') {
    ...
    const result = await writeJsonFile(env, PATH, payload.items, payload.commitMessage || 'Update Charlotte Map via dashboard');
    if (!result.ok) return json(result.status, { error: result.error, detail: result.detail });
