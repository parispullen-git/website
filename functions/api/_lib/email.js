/* Resend transactional email helper for order-status notifications.
   Cloudflare Workers port of netlify/functions/lib/email.js -- identical
   logic (a plain fetch to Resend's API), just reading the API key off the
   per-request `env` object instead of process.env. */
...
async function sendOrderStatusEmail(env, order) {
