/* Read/write for the real Case Files -- data/casefiles.json, which
   build_casefiles.py bakes into casefiles.html. See journal-live.js for
   the fuller explanation this mirrors: GitHub Contents API only, never
   the live site directly, and Deploy Live still has to run to publish. */
...
const PATH = 'data/casefiles.json';
...
export async function onRequest(context) {
  ...
  if (event.httpMethod === 'POST') {
    ...
    const result = await writeJsonFile(env, PATH, payload.items, payload.commitMessage || 'Update Case Files via dashboard');
    if (!result.ok) return json(result.status, { error: result.error, detail: result.detail });
