/* Clients are not their own stored collection -- this rolls one up live
   ...
   Cloudflare Pages Functions port of netlify/functions/clients.js. */

const { listRecords, checkAuth, json } = require('./_lib/store');
...
export async function onRequest(context) {
  ...
  const proposals = await listRecords(env, 'proposals');
  proposals.forEach((p) => {
  ...
  const inquiries = await listRecords(env, 'inquiries');
  inquiries.forEach((i) => {
