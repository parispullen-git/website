...
exports.handler = async function (event) {
  ...
  if (!key) {
    return { statusCode: 500, body: JSON.stringify({ error: 'Stripe is not configured on the server.' }) };
  }
  ...
  const expectedHash = process.env.INVOICE_PASSPHRASE_HASH;
  ...
  if (!name || !email || !allValid) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Missing or invalid invoice details.' }) };
  }
