#!/usr/bin/env python3
"""Apply approved Kitchen, Music Lounge, Bedroom and Closet v7 artifact maps."""
import json
import re
...
MUSIC_ARTS = [
    artifact("artwork", "The Artwork", "48.4%", "15.0%", "The artwork wall above the record console — part gallery, part reference library.", (("Collection", "Paris Pullen"),)),
    artifact("recordplayer", "The Record Player", "38.6%", "34.1%", "ATF to OVO — the complete list, queued on shuffle and left running.", (("Plays", "One playlist, shuffled"), ("Manual skips", "Yes"))),
    artifact("journal", "The Journal", "39.4%", "59.9%", "The journal rests on the coffee table beside the books and candlelight.", (("Read it", "The Journal"),)),
    artifact("cocktails", "The Gentlemen’s Cocktail Guide", "72.1%", "70.3%", "The house cocktail guide sits within reach of the sofa — the right drink without leaving the room.", (("House classics", "Eleven"),)),
]

BEDROOM_ARTS = [
    artifact("artwork", "The Artwork", "36.8%", "17.4%", "The portrait beside the fireplace anchors the room’s personal collection.", (("Collection", "Paris Pullen"),)),
    artifact("suit", "The Blueprint Game", "63.9%", "37.5%", "The look laid across the bed opens The Blueprint — choose the combination, then own how you show up.", (("Experience", "The Blueprint"),)),
    artifact("cocktails", "The Gentlemen’s Cocktail Guide", "16.2%", "46.1%", "The house cocktail guide sits beside the lounge seating for a proper nightcap without leaving the suite.", (("House classics", "Eleven"),)),
    artifact("journal", "The Journal", "85.9%", "69.3%", "The open journal on the writing desk — a private note before the room goes quiet.", (("Read it", "The Journal"),)),
]

# Image-relative points from the approved 2048x1152 Closet v7 annotation.
# City Guide is a separate animated portal. Boxer uses the existing After Hours game portal.
CLOSET_ARTS = [
    artifact("suits", "The Blueprint Game", "14.8%", "31.0%", "The Blueprint opens from the wardrobe itself — choose the combination, then own how you show up.", (("Experience", "The Blueprint"),)),
    artifact("boxer", "The After Hours Boxing Game", "46.2%", "22.5%", "The gloves on the center shelving open After Hours — the boxing challenge after dark.", (("Experience", "After Hours"),)),
    artifact("journal", "The Journal", "59.0%", "15.5%", "The journal sits within the dressing room — notes made before the next look leaves the room.", (("Read it", "The Journal"),)),
]
...
def patch_runtime():
    ...
    'bath': ["""),
        (r"    'closet': \[.*?\n    \],\n    'kitchen': \[", """    'closet': [
      { key:'suits', name:'The Blueprint Game', x:'14.8%', y:'31.0%', body:'The Blueprint opens from the wardrobe itself — choose the combination, then own how you show up.', specs:[['Experience', 'The Blueprint']] },
      { key:'boxer', name:'The After Hours Boxing Game', x:'46.2%', y:'22.5%', body:'The gloves on the center shelving open After Hours — the boxing challenge after dark.', specs:[['Experience', 'After Hours']] },
      { key:'journal', name:'The Journal', x:'59.0%', y:'15.5%', body:'The journal sits within the dressing room — notes made before the next look leaves the room.', specs:[['Read it', 'The Journal']] },
    ],
    'kitchen': ["""),
    ]
    for pattern, replacement in replacements:
        ...
        if count != 1:
            raise RuntimeError(f"Could not replace runtime block: {pattern}")

    # Reuse the existing After Hours game portal rendering for the Closet boxer node.
    text = text.replace("if (room.id === 'gym' && a.key === 'boxer')", "if ((room.id === 'gym' || room.id === 'closet') && a.key === 'boxer')")
    text = text.replace("return !(room.id === 'gym' && a.key === 'boxer');", "return !((room.id === 'gym' || room.id === 'closet') && a.key === 'boxer');")
    ...
    text = set_city_guide(text, "closet", "34.8%", "8.6%", js=True)
...
def patch_build():
    ...
    text = text.replace('if f["id"] == "gym" and key == "boxer":', 'if f["id"] in ("gym", "closet") and key == "boxer":')
    text = re.sub(r'\n\s*"music-lounge":\s*\("50%",\s*"62%"\),?', "", text, count=1)
    text = set_city_guide(text, "music-lounge", "5.5%", "27.3%", js=False)
    text = set_city_guide(text, "bedroom", "14.7%", "14.4%", js=False)
    text = set_city_guide(text, "closet", "34.8%", "8.6%", js=False)
...
def main():
    ...
    print("Approved Kitchen + Music Lounge + Bedroom + Closet v7 artifacts applied")
