#!/usr/bin/env python3
"""
Merges picks4.json (the "150 more" batch, 161 candidates / 149 unique handles)
+ colors4.tsv (manually identified colors) into wardrobe_data.py's WARDROBE list.
"""
import json
import re
import sys
...
COLOR_MAP = {
    ...
    "slate-blue": "Slate Blue", "sky-blue": "Sky Blue", "mint": "Mint", "khaki": "Khaki",
    "copper": "Copper", "turquoise": "Turquoise", "rose-gold": "Rose Gold",
    "charcoal-gold": "Charcoal/Gold", "navy-gold": "Navy/Gold", "white-gold": "White/Gold",
    "red-green": "Red/Green", "red-black": "Red/Black",
}
...
def main():
    picks = json.loads((SCRATCH / "picks4.json").read_text())
    colors = {}
    for line in (SCRATCH / "colors4.tsv").read_text().splitlines():
        if not line.strip():
            continue
        idx, slug = line.split("\t")
        colors[int(idx)] = slug
    ...
    blocks = ["\n  # ---------- EXPANDED BATCH 4 (100 more, batch 4, auto-generated) ----------"]
    cur_cat = None
    for it in new_items:
        if it["cat"] != cur_cat:
            cur_cat = it["cat"]
