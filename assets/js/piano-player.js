...
(function () {
  ...
  /* ----------------------------------------------------------------
     Ambient per-room Spotify autoplay (Music Lounge sting+playlist and
     every other room's single assigned track) has been removed -- the
     hidden-host containment trick (.room-track-host, position:fixed;
     left:-9999px) didn't reliably keep Spotify's embed chrome off
     screen, so paging between rooms was surfacing a full-width playback
     bar at the bottom of the viewport. window.PPAmbient stays defined
     (tv-remote.js's Suite Remote and artifact-experiences.js's record
     shelf both read it) but now always reports "nothing playing" --
     the Piano artifact's own in-drawer player below is unaffected,
     since that's a deliberately visible, user-opened widget rather
     than something trying to stay hidden. */
  window.PPAmbient = {
